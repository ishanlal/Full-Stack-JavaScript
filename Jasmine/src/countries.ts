...
async function getCountry(name: string) {
  const getApi = await axios.get(`https://restcountries.com/v2/name/${name}`);
...
async function getRegionCountries(regionalbloc: string) {
  const getApi = await axios.get(`https://restcountries.com/v2/regionalbloc/${regionalbloc}`);
...
async function getRegionCapitals(regionalbloc: string) {
  ...
  const promises = countryEndPoints.map((endpoint: any) => axios.get(`https://restcountries.com/v2/name/${endpoint}`));
